...
async function updateStudentStatus({ email }) {
  const query =
    'UPDATE users SET status = \'NOTACTIVE\' WHERE email = $1 AND status = \'ACTIVE\'';
...
async function findAdminByUsername(username) {
  ...
  return new Promise((resolve, reject) => {
    db.query(query, [username], (err, result) => {
      ...
      resolve(result.rows[0] || null); // Return admin record or null
    });
  });
}
...
async function findUserByUsername(username) {
  const query = 'SELECT * FROM access WHERE username = $1';
  return new Promise((resolve, reject) => {
    db.query(query, [username], (err, result) => {
      if (err) {
        return reject(err);
      }
      resolve(result.rows[0] || null); // Return user record or null
    });
  });
}
...
async function addUser(username, password) {
  const query = 'INSERT INTO access (username, password) VALUES ($1, $2)';
  return new Promise((resolve, reject) => {
...
async function updateAccessPassword(username, newPassword) {
  const query = 'UPDATE access SET password = $1 WHERE username = $2';
  return new Promise((resolve, reject) => {
